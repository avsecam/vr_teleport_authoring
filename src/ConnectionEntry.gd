...
@export var connected_to: TeleportNode:
	set(value):
		$AreaName.text = value.area_name
		connected_to = value

func _ready():
	$AreaName.pressed.connect(_on_area_name_button_pressed)
	$Delete.pressed.connect(_on_delete_button_pressed)
	$Locks.pressed.connect(_on_locks_button_pressed)
	
	$AreaName.text = connected_to.area_name if connected_to else ""

func _process(_delta):
	$Delete.visible = teleporter != null
